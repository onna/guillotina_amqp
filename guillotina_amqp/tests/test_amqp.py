...
async def test_add_task(dummy_request, amqp_worker_rabbitmq, amqp_channel,
    ...
    main_queue = await amqp_worker_rabbitmq.queue_main(amqp_channel)
    assert main_queue['message_count'] == 0

    aiotask_context.set('request', None)


async def test_generator_tasks(dummy_request, amqp_worker_rabbitmq, amqp_channel,
                               configured_state_manager):
    aiotask_context.set('request', dummy_request)
...
async def test_run_task(dummy_request, amqp_worker_rabbitmq):
    aiotask_context.set('request', dummy_request)
    state = await add_task(_test_func, 1, 2)
    await state.join(0.01)
    await asyncio.sleep(0.1)  # prevent possible race condition here
    assert amqp_worker_rabbitmq.total_run == 1
    assert await state.get_result() == 3
    aiotask_context.set('request', None)


async def test_task_from_service(amqp_worker_rabbitmq, container_requester):
...
async def test_task_commits_data_from_service(amqp_worker_rabbitmq, container_requester):
    async with container_requester as requester:
...
async def test_async_gen_task_commits_data_from_service(configured_state_manager,
                                                        amqp_worker_rabbitmq, container_requester):
    async with container_requester as requester:
        await requester('POST', '/db/guillotina', data=json.dumps({
            '@type': 'Item',
            'id': 'foobar',
...
async def test_cancels_long_running_task(dummy_request, amqp_worker_rabbitmq, configured_state_manager):
    aiotask_context.set('request', dummy_request)
...
async def test_decorator_task(dummy_request, amqp_worker_rabbitmq):
    aiotask_context.set('request', dummy_request)
...
async def test_errored_job_should_be_published_to_delayed_queue(dummy_request,
    ...
    await ts.join(0.1)
    assert amqp_worker_rabbitmq.total_run == 1
    await amqp_worker_rabbitmq.join()
    state = await ts.get_state()
    ...
    delayed = await amqp_worker_rabbitmq.queue_delayed(amqp_channel)
...
async def test_worker_retries_should_not_exceed_the_limit(dummy_request,
    ...
    max_retries = amqp_worker_rabbitmq.max_task_retries
    await amqp_worker_rabbitmq.state_manager.update(ts.task_id, {
    ...
    main_queue = await amqp_worker_rabbitmq.queue_main(amqp_channel)
    errored_queue = await amqp_worker_rabbitmq.queue_errored(amqp_channel)
...
async def test_worker_beacons_process_exit(dummy_request,
    ...
    # being sent.
    await amqp_worker_rabbitmq.stop()
